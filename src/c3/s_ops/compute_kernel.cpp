...
Compute_Kernel::Compute_Kernel() : c3::Subject(){
...
int Compute_Kernel::subj_specific_load(std::string subj){
  ...
  for(int fiber_idx = 0; fiber_idx < num_tracks; ++fiber_idx){
    ...
    for(int i = 0; i < s1; ++i){
      ...
      for(int j = 0; j < s2; ++j){
        ...
        output[qtd_vector_1[i].idx + (hemi_1 * N_mesh_1) - 1] //.m files are 1 indexed
...
int Compute_Kernel::save_file(std::string filename){
  ...
  for(int i = 0; i < N_output; ++i){
    for(int j = 0; j < N_output; ++j){
      if(j==i)
        continue;
      temp = (output[i][j] + output[j][i]) * (1.0/num_nonnull_tracks);
      if(temp > final_thold){
...
int Compute_Kernel::load_file(std::string filename){
  return 1;
}
